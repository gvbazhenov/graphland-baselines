...
EXPERIMENT_RESULTS_ROOT = '../experiments'

os.makedirs(EXPERIMENT_RESULTS_ROOT, exist_ok=True)
        ...
        for test_group_id in range(len(self.multi_handler.tst_handlers_group)):
            ...
            for i, handler in enumerate(tst_handlers):
                    ...
                    for met in reses:
                        ...
                        tot_reses[met + '_std'] = float(tem_arr.std())
                        tot_reses[met + '_mean'] = float(tem_arr.mean())
                    ...
                    results_path = f'{EXPERIMENT_RESULTS_ROOT}/{handler.data_name}.yaml'
                    _save_dict(results_path, tot_reses)
    ...
    def test_epoch(self, handler, dataset_id):
        with t.no_grad():
            ...
            for i, batch_data in enumerate(tst_loader):
                ...
                hit = (labels == t.argmax(preds, dim=-1)).float().sum().item()
                ep_acc += hit
                ep_tot += labels.shape[0]
                log('Steps %d/%d: hit = %d, tot = %d          ' % (i, steps, ep_acc, ep_tot), save=False, oneline=True)
        ret = dict()

        probas = all_preds.cpu().numpy()
        labels = all_labels.cpu().numpy()
        predictions = np.argmax(probas, axis=1)
        if probas.shape[1] == 2: ret['ap'] = float(average_precision_score(labels, probas[:, 1]))
        else: ret['accuracy'] = float(accuracy_score(labels, predictions))

        ret['Acc'] = ep_acc / ep_tot
        ret['F1'] = f1_score(all_labels.cpu().numpy(), predictions, average='macro')
        ret['tstNum'] = ep_tot
        t.cuda.empty_cache()
        return ret
...
if __name__ == '__main__':
    ...
    datasets['graphland'] = os.listdir(GRAPHLAND_DATA_ROOT)
    ...
    elif args.dataset_setting in datasets['graphland']:
        trn_datasets = tst_datasets = [args.dataset_setting]
