...
class Dataset(Generic[T]):
    ...
    def __post_init__(self):
        ...
        for key in ['X_num', 'X_bin']:
            if key in self.data:
                x = self.data[key]['train']
                is_torch = isinstance(x, torch.Tensor)
                device = x.device if is_torch else None
                if is_torch: x = x.cpu().numpy()

                imputer = sklearn.impute.SimpleImputer(strategy='most_frequent')
                imputer.fit(x)

                for part, x in self.data[key].items():
                    if is_torch: x = x.cpu().numpy()
                    x = imputer.transform(x)
                    if is_torch: x = torch.from_numpy(x).to(device)
                    self.data[key][part] = x
...
def transform_num(X: NumpyDict, policy: NumPolicy, seed: Optional[int]) -> NumpyDict:
    ...
    # X_transformed = {}
    # for k, v in X.items():
    #     _df = pd.DataFrame(normalizer.transform(v))
    #     X_transformed[k] = _df.fillna(_df.mean()).values
