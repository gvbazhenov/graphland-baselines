...
def main(
    ...
    if dataset.X_num is None:
        ...
        X = {k: pd.DataFrame(v.astype(np.int32)) for k, v in dataset.X_cat.items()}
        ...
        X = {
            part: pd.concat(
                [
                    ...
                    pd.DataFrame(
                        dataset.X_cat[part].astype(np.int32),
    ...
    if dataset.X_cat is not None:
        model_extra_kwargs['cat_feature'] = list(
    ...
    # dataset_info = dataset.Y
    # lib.dump_dataset_info(dataset_info, output)
