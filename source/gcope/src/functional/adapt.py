...
def run(
    ...
    model.load_state_dict(torch.load(pretrained_file, map_location='cpu'), strict=False)
    model = model.cuda()
...
def finetune(
    ...
    from torchmetrics import MeanMetric, Accuracy, F1Score, AUROC, AveragePrecision
    from tqdm import tqdm
    from copy import deepcopy

    loss_metric = MeanMetric().to(device)
    print(f'{model.answering.num_class=}')

    # NOTE: add average precision metric - done
    ap_metric = (
        AveragePrecision(task='binary').to(device)
        if model.answering.num_class == 2 else None
    )
    ...
    for e in range(epoch):
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                # NOTE: add average precision metric - done
                if ap_metric is not None:
                    ap_metric.update(probas[:, 1], batch.y)

                acc_metric.update(pred, batch.y)
                f1_metric.update(pred, batch.y)
                auroc_metric.update(probas, batch.y)
    ...
    return {
        ...
        {'ap': ap_metric.compute().item()} if ap_metric is not None else {}
    )
