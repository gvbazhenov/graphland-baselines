...
def compare_layers():
    ...
    layer1 = GATConv(in_channels=channel, out_channels=channel, heads=1, bias=False, add_self_loops=False).to(DEVICE)
    layer2 = GATConv(in_channels=channel, out_channels=channel, heads=1, bias=False, add_self_loops=False).to(DEVICE)

    tri_layer1.aggr_lin.weight = layer1.lin.weight
    tri_layer1.att_src.weight.data = layer1.att_src.data.squeeze(dim=1)
    tri_layer1.att_dst.weight.data = layer1.att_dst.data.squeeze(dim=1)

    tri_layer2.aggr_lin.weight = layer2.lin.weight
    tri_layer2.att_src.weight.data = layer2.att_src.data.squeeze(dim=1)
    tri_layer2.att_dst.weight.data = layer2.att_dst.data.squeeze(dim=1)
    ...
    u, v = edge_index
    logits = new_x[u] @ layer1.att_src.data.squeeze(dim=1).T + new_x[v] @ layer1.att_dst.data.squeeze(dim=1).T
    ...
    u, v = edge_index
    logits = new_x[u] @ layer2.att_src.data.squeeze(dim=1).T + new_x[v] @ layer2.att_dst.data.squeeze(dim=1).T
    logits = F.leaky_relu(logits, negative_slope=0.2)
    weight = sparse_softmax(logits, v)  # Normalize over incoming node
    mat_out = torch.zeros_like(new_x)
    mat_out = scatter(weight * new_x[u], v, dim=0, out=mat_out)  # Weight and sum
    ...
    assert torch.allclose(tri_out, out, rtol=5e-3), f"Triton GAT forward does not work with regular"
    print(f"Triton GAT forward works!")
    ...
    print("Triton Output:", tri_out[:5])
    print("Regular Output:", out[:5])
    assert torch.allclose(tri_out, out, rtol=5e-3), f"Triton GAT backward does not work"
    print(f"Triton GAT backward works!\n")
